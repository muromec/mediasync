...
public class JmDNS {
   ...
   public JmDNS() throws IOException {
      ...
      try {
         ...
         init(addr.isLoopbackAddress() ? null : addr, addr.getHostName()); // [PJYF
         // Oct
         // 14
         // 2004]
         // Why
    ...
    * Request service information. The information about the service is
    * requested
    * and the ServiceListener.resolveService method is called as soon as it is
    * available.
    * 
    * @param type
    *           full qualified service type, such as
   ...
   void handleServiceResolved(ServiceInfo info) {
      ...
      if (list != null) {
         ServiceEvent event = new ServiceEvent(this, info.type, info.getName(),
               info);
   ...
   public void addServiceTypeListener(ServiceTypeListener listener)
      ...
      for (Iterator iterator = serviceTypes.values().iterator(); iterator
            .hasNext();) {
         listener.serviceTypeAdded(new ServiceEvent(this, (String) iterator
               .next(), null, null));
      }
         ...
         if ((type.indexOf("._mdns._udp.") < 0)
            ...
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
               ((ServiceTypeListener) iterator.next())
                     .serviceTypeAdded(new ServiceEvent(this, type, null, null));
            }
         }
      }
   }
    ...
    * The state of each service info of the host name is advanced, when a probe
    * has
    * been sent for it.
    ...
    * affected host name) are taken away from the prober. This eventually causes
    * the prober tho cancel itself.
    */
   private class Prober extends TimerTask {
      ...
      public void start() {
         ...
         if (state == DNSState.ANNOUNCED
            ...
            timer.schedule(this, random
                  .nextInt(1 + DNSConstants.PROBE_WAIT_INTERVAL),
               ...
               for (Iterator i = list.iterator(); i.hasNext();) {
                  ...
                  synchronized (info) {
                     if (info.getState() == taskState && info.task == this) {
                        ...
                        if (out == null) {
                           out = new DNSOutgoing(DNSConstants.FLAGS_QR_QUERY);
                           out.addQuestion(new DNSQuestion(info
                                 .getQualifiedName(), DNSConstants.TYPE_ANY,
                                 DNSConstants.CLASS_IN));
                        }
                        out.addAuthorativeAnswer(new DNSRecord.Service(info
                              .getQualifiedName(), DNSConstants.TYPE_SRV,
                              DNSConstants.CLASS_IN, DNSConstants.DNS_TTL,
                              info.priority, info.weight, info.port, localHost
                                    .getName()));
                     }
                  }
               }
               ...
               synchronized (info) {
                  if (info.getState() == taskState && info.task == this) {
                     ...
                     out.addAnswer(new DNSRecord.Pointer(info.type,
                           ...
                           DNSConstants.DNS_TTL, info.getQualifiedName()), 0);
                     out.addAnswer(new DNSRecord.Service(info
                           .getQualifiedName(), DNSConstants.TYPE_SRV,
                           DNSConstants.CLASS_IN, DNSConstants.DNS_TTL,
                           info.priority, info.weight, info.port, localHost
                                 .getName()), 0);
                     out.addAnswer(new DNSRecord.Text(info.getQualifiedName(),
                           DNSConstants.TYPE_TXT, DNSConstants.CLASS_IN,
                           DNSConstants.DNS_TTL, info.text), 0);
                  }
               }
            }
            if (out != null) {
         ...
         for (Iterator i = in.questions.iterator(); i.hasNext();) {
            ...
            if (entry instanceof DNSQuestion) {
               ...
               iAmTheOnlyOne &= (q.type == DNSConstants.TYPE_SRV
                     ...
                     || localHost.getName().equalsIgnoreCase(q.name) || services
                     .containsKey(q.name.toLowerCase()));
               if (!iAmTheOnlyOne) {
                  break;
               }
            }
         ...
         synchronized (ioLock) {
            ...
            if (state == DNSState.ANNOUNCED) {
               try {
                  ...
                  // Answer questions
                  for (Iterator iterator = in.questions.iterator(); iterator
                        .hasNext();) {
                     ...
                     if (entry instanceof DNSQuestion) {
                        ...
                        if (type == DNSConstants.TYPE_ANY
                              || type == DNSConstants.TYPE_SRV) { // I ama not
                           // sure of why
                           // there is a
                           // special
                           // case here
                           ...
                           if (localHost.getName()
                              ...
                              if (serviceTypes.containsKey(q.getName()
                                    .toLowerCase())) {
                        ...
                        switch (type) {
                           ...
                           case DNSConstants.TYPE_PTR: {
                              ...
                              for (Iterator serviceIterator = services.values()
                                    .iterator(); serviceIterator.hasNext();) {
                                 ServiceInfo info = (ServiceInfo) serviceIterator
                                       .next();
                                 if (info.getState() == DNSState.ANNOUNCED) {
                                    if (q.name.equalsIgnoreCase(info.type)) {
                                       ...
                                       answers.add(new DNSRecord.Pointer(
                                             info.type, DNSConstants.TYPE_PTR,
                                             DNSConstants.CLASS_IN,
                                             DNSConstants.DNS_TTL, info
                                                   .getQualifiedName()));
                                       answers.add(new DNSRecord.Service(info
                                             .getQualifiedName(),
                                             DNSConstants.TYPE_SRV,
                                             DNSConstants.CLASS_IN
                                                   | DNSConstants.CLASS_UNIQUE,
                                             DNSConstants.DNS_TTL,
                                             info.priority, info.weight,
                                             info.port, localHost.getName()));
                                       answers.add(new DNSRecord.Text(info
                                             .getQualifiedName(),
                              ...
                              if (q.name
                                    .equalsIgnoreCase("_services._mdns._udp.local.")) {
                                 for (Iterator serviceTypeIterator = serviceTypes
                                       .values().iterator(); serviceTypeIterator
                                       .hasNext();) {
                                    answers.add(new DNSRecord.Pointer(
                           ...
                           case DNSConstants.TYPE_TXT: {
                              ServiceInfo info = (ServiceInfo) services
                                    .get(q.name.toLowerCase());
                              if (info != null
                                 ...
                                 answers.add(new DNSRecord.Pointer(info.type,
                                       DNSConstants.TYPE_PTR,
                                       DNSConstants.CLASS_IN,
                                       DNSConstants.DNS_TTL, info
                                             .getQualifiedName()));
                                 answers.add(new DNSRecord.Service(info
                                       ...
                                       info.weight, info.port, localHost
                                             .getName()));
                                 answers.add(new DNSRecord.Text(info
                                       .getQualifiedName(),
                                       DNSConstants.TYPE_TXT,
                  ...
                  if (answers.size() != 0) {
                     ...
                     for (Iterator i = answers.iterator(); i.hasNext();) {
                        out = addAnswer(in, addr, port, out, (DNSRecord) i
                              .next());
                     }
                     send(out);
                  }
            ...
            if (state == DNSState.ANNOUNCED) {
               if (++count < 3) {
                  ...
                  out.addQuestion(new DNSQuestion(
                        "_services._mdns._udp.local.", DNSConstants.TYPE_PTR,
                        DNSConstants.CLASS_IN));
                  for (Iterator iterator = serviceTypes.values().iterator(); iterator
                        .hasNext();) {
                     out.addAnswer(new DNSRecord.Pointer(
                           "_services._mdns._udp.local.",
                           DNSConstants.TYPE_PTR, DNSConstants.CLASS_IN,
                           DNSConstants.DNS_TTL, (String) iterator.next()), 0);
                  }
                  send(out);
               } else {
         ...
         try {
            if (state == DNSState.ANNOUNCED) {
               if (count++ < 3) {
                  ...
                  long now = System.currentTimeMillis();
                  DNSOutgoing out = new DNSOutgoing(DNSConstants.FLAGS_QR_QUERY);
                  out.addQuestion(new DNSQuestion(type, DNSConstants.TYPE_PTR,
                        DNSConstants.CLASS_IN));
                  for (Iterator s = services.values().iterator(); s.hasNext();) {
         ...
         try {
            if (state == DNSState.ANNOUNCED) {
               if (count++ < 3 && !info.hasData()) {
                  long now = System.currentTimeMillis();
                  DNSOutgoing out = new DNSOutgoing(DNSConstants.FLAGS_QR_QUERY);
                  out.addQuestion(new DNSQuestion(info.getQualifiedName(),
                  ...
                  out.addAnswer((DNSRecord) cache.get(info.getQualifiedName(),
   ...
   private class Canceler extends TimerTask {
      ...
      int count = 0;
      /**
       * The services that need canceling.
       ...
       * state ANNOUNCED to state CANCELED before we get here. We could get rid
       * of this lock, if we added a state named CANCELLING to DNSState.
       */
      private Object lock;
      int ttl = 0;
      ...
      public Canceler(Collection infos, Object lock) {
         this.infos = (ServiceInfo[]) infos.toArray(new ServiceInfo[infos
               .size()]);
         this.lock = lock;
      }
         ...
         try {
            if (++count < 3) {
               ...
               for (int i = 0; i < infos.length; i++) {
                  ...
                  out.addAnswer(new DNSRecord.Pointer(info.type,
                        DNSConstants.TYPE_PTR, DNSConstants.CLASS_IN, ttl, info
                              .getQualifiedName()), 0);
                  out.addAnswer(new DNSRecord.Service(info.getQualifiedName(),
                        DNSConstants.TYPE_SRV, DNSConstants.CLASS_IN, ttl,
                        info.priority, info.weight, info.port, localHost
                              .getName()), 0);
                  out.addAnswer(new DNSRecord.Text(info.getQualifiedName(),
                        DNSConstants.TYPE_TXT, DNSConstants.CLASS_IN, ttl,
                        info.text), 0);
                  DNSRecord answer = localHost.getDNS4AddressRecord();
                  if (answer != null) {
                     out.addAnswer(answer, 0);
      ...
      if (DNSState.CANCELED != state) {
         synchronized (this) { // Synchronize only if we are not already in
            ...
            try {
               ...
               logger.log(Level.WARNING, "recover() Start services exception ",
                     exception);
            }
   ...
   private static class ServiceCollector implements ServiceListener {
      ...
      public ServiceInfo[] list() {
         synchronized (infos) {
            return (ServiceInfo[]) infos.values().toArray(
                  new ServiceInfo[infos.size()]);
         }
      }
